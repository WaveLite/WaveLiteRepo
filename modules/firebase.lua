...
local function registerSession()
	pcall(function()
		local url  = DB_URL .. "sessions/" .. SESSION_ID .. ".json"
		local body = '{"ts":' .. tostring(os.time()) .. ',"user":"' .. jsonEscape(myDisplay) .. '"}'
...
local function removeSession()
	pcall(function()
		httpRequest("DELETE", DB_URL .. "sessions/" .. SESSION_ID .. ".json", nil)
	end)
end
...
local function saveTimeout(until_, reason, strikes)
	pcall(function()
		local body = '{"until":'..tostring(until_)..',"reason":"'..jsonEscape(reason)..'","strikes":'..tostring(strikes)..'}'
		httpRequest("PUT", DB_URL.."timeouts/"..myUsername..".json", body)
	end)
end
...
local function modTimeoutUser(username, seconds, reason)
	pcall(function()
		...
		local body = '{"until":'..tostring(until_)..',"reason":"'..jsonEscape(reason)..'","strikes":'..tostring(strikes)..'}'
		httpRequest("PUT", DB_URL.."timeouts/"..username..".json", body)
	end)
end
 
local function modChatBanUser(username, reason)
	pcall(function()
		local body = '{"banned":true,"reason":"'..jsonEscape(reason)..'"}'
		httpRequest("PUT", DB_URL.."bans/"..username..".json", body)
	end)
end
...
local function modScriptBanUser(username, reason)
	pcall(function()
		local body = '{"banned":true,"reason":"'..jsonEscape(reason)..'"}'
		httpRequest("PUT", DB_URL.."script_bans/"..username..".json", body)
	end)
end
...
local function fetchDeletedIds()
	...
	pcall(function()
		...
		if not res or not res.Body or res.Body == "null" or res.Body == "" then return end
		for id in res.Body:gmatch('"([^"]+)":%{"deleted":true%}') do
			deleted[id] = true
		end
	end)
	return deleted
...
return {
	...
	myDisplay               = myDisplay,
	getMyRole               = function() return myRole end,
	getChatBanned           = function() return chatBanned end,
	getScriptBanned         = function() return scriptBanned end,
	getSpamStrikes          = function() return spamStrikes end,
	setSpamStrikes          = function(v) spamStrikes = v end,
	getLocalTimeoutUntil    = function() return localTimeoutUntil end,
	setLocalTimeoutUntil    = function(v) localTimeoutUntil = v end,
	getRecentMessages       = function() return recentMessages end,
	getLastSentText         = function() return lastSentText end,
	setLastSentText         = function(v) lastSentText = v end,
	setChatBanned           = function(v) chatBanned = v end,
